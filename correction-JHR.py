...
# Je vais donc identifier chacune de ces balises "tr" pour en extraire l'hyperlien qu'il contient à même sa balise "href".
    for ligne in page.find_all("tr"):
       ...
       # On attribue une variable à une liste vide, qui nous permettra de rassembler l'information que nous désirerons transposer dans le .csv final.
        ...
        # i =+ 1 ### J'avais fait une erreur, ici: c'est += et non =+
...
### Excellent script, qui produit un CSV que tu pourrais éditer manuellement pour décaler les lignes ne contenant que 8 colonnes.
### Sinon, la façon de contourner ce problème aurait pu être la suivante:

            ### D'abord, je vérifie combien il y a d'éléments dans chacun des contrats en créant une liste que je peux appeler «items»
            items = detail.find_all("tr")
            print(len(items)) ### Affichage me permettant de constater qu'il n'y a que deux cas possibles: 8 ou 10 éléments

            ### J'ajoute ensuite chacun des éléments en fonction de leur position dans la liste «items»
            ### D'abord les cinq premiers, toujours les mêmes, que le contrat compte 8 ou 10 éléments
            for n in range(0,5):
                contratAgenceSpatiale.append(items[n].td.text)

            ### Ensuite, après avoir sauté l'élément [5] (la date de livraison, toujours vide)
            ...
            if len(items) == 10:
                ...
                contratAgenceSpatiale.append(items[6].td.text)
                contratAgenceSpatiale.append("0,00 $") ### Valeur des modifications; dans les cas où il n'y a que 8 éléments, c'est toujours zéro
                contratAgenceSpatiale.append(items[6].td.text) ### On répète la ligne [6], car la valeur initiale et la valeur totale du contrat sont la même 
                contratAgenceSpatiale.append(items[7].td.text)

            print(contratAgenceSpatiale)

            ouverture = open("fichier-csv-JHR.csv","a")
            fonctionecriture = csv.writer(ouverture)
            fonctionecriture.writerow(contratAgenceSpatiale)
